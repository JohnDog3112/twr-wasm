...
export class twrConsoleScreen extends twrLibrary implements ICanvasEvents {
   ...
   jsSpawnWindow(width: number = 500, height: number = 500, x?: number, y?: number): twrConsoleWindow {
   ...
   twrScreenSpawnWindow(mod: IWasmModule | IWasmModuleAsync, width?: number, height?: number, x?: number, y?: number): number {
   ...
   jsSpawnPopupWindow(window: twrConsoleWindow, width: number = 50, height: number = 50, x?: number, y?: number): twrConsoleWindow {
      ...
      if (childSpawnOffset.x+middleX >= this.canvas.width-50) {
         childSpawnOffset.x = 0;
         childSpawnOffset.y = 0;
         childSpawnOffset.baseX = childSpawnOffset.x;
      }
      if (childSpawnOffset.y+middleY >= this.canvas.height-50) {
         childSpawnOffset.y = 0;
      ...
      if (offsetSpawnX <= 0) {
         spawnX = 0;
      } else if (offsetSpawnX+width >= this.canvas.width) {
         spawnX = this.canvas.width-width;
      }
      let spawnY = offsetSpawnY+height;
      if (offsetSpawnY <= 0) {
         spawnY = 0;
      } else if (offsetSpawnY+height >= this.canvas.height) {
         spawnY = this.canvas.height-height;
      }
